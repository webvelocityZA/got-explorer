...
const HouseList: React.FC<HouseListProps> = ({
  ...
  return (
    <nav aria-label="House list" className="md:h-[100vh]">
      <ul className="grid grid-cols-5 flex-row h-[35vh] md:h-[100vh]">
        {keyHouses.map((house) => (
          <li
            key={house.id}
          ...
          >
            <div className="flex items-center justify-center hover:shadow-lg transition-shadow md:max-w-fit cursor-pointer">
              <img
                src={getImage(house.id, "house")}
                alt={house.name}
                className="w-full h-[35vh] md:h-screen md:max-h-screen object-cover"
              />
            </div>
          </li>
        ))}
